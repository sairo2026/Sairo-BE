-- 운영 DB는 도메인 테이블 전체가 0행이라 기존 데이터 보존 없이 재구성한다.
...
COMMENT ON TABLE property_contract IS '매물의 현재·과거 계약 이력. 매물과 분리해 저장하므로 재계약 시 기존 계약을 수정하지 않고 새 행을 만든다';
...
COMMENT ON CONSTRAINT ck_property_contract_sale_terminal ON property_contract IS '매매는 재계약(RENEWED)·중도종료(TERMINATED)로 전이할 수 없다 — 기간 개념이 없는 1회성 거래이므로 거래완료·취소로만 종료';
...
COMMENT ON COLUMN coordination.cancel_reason IS '취소 사유(선택 — 사유 불문 취소 가능하므로 필수 아님)';
...
COMMENT ON CONSTRAINT ck_coordination_cancelled ON coordination IS 'CANCELLED 상태와 취소 시각은 항상 함께 존재하거나 함께 없다. 완료 후 취소로 전이해도 scheduled_at·confirmed_at은 건드리지 않으므로 위반되지 않는다';
...
COMMENT ON TABLE coordination_status_history IS '조율현황의 모든 전이를 기록하는 감사 이력. 확정률은 현재 상태가 아니라 이 이력에서 COMPLETED 도달 여부로 계산한다 — COMPLETED에서 CANCELLED로 다시 전이할 수 있기 때문이다. to_status 값 자체는 CHECK로 제한하지만 from→to 전이 조합(예: COMPLETED에서 TENANT_CHECKING으로 되돌아가는 조합)은 DB가 막지 않는다 — 조율현황 전이표(API 명세서 5장)를 서비스 계층이 같은 트랜잭션에서 강제한다';
...
COMMENT ON INDEX uq_user_pending_office IS '사용자당 진행 중(PENDING) 참여 요청은 최대 1개 — 동시에 하나의 등록·참여 요청만 허용한다는 정책의 나머지 절반';
