...
class RateLimitCounterTest extends AbstractSchemaTest {
  ...
  void 같은_bucket_key를_여러_커넥션이_동시에_UPSERT해도_요청수를_잃어버리지_않는다() throws Exception {
    ...
    try {
      java.util.List<Future<Integer>> futures =
      ...
      assertThat(startBarrier.await(5, TimeUnit.SECONDS))
          .as("모든 스레드가 커넥션을 얻고 대기 상태까지 도달해야 함")
          .isTrue();
      ...
      assertThat(observedCounts)
          .as("동시에 들어온 %d개 UPSERT가 반환한 request_count는 서로 달라야 함(직렬화 증거)", concurrentCalls)
          .hasSize(concurrentCalls)
          .doesNotHaveDuplicates()
          .containsExactlyInAnyOrderElementsOf(
              IntStream.rangeClosed(1, concurrentCalls).boxed().toList());
        ...
        try (PreparedStatement ps =
          ...
          try (ResultSet rs = ps.executeQuery()) {
            ...
            assertThat(rs.getInt(1))
                .as("최종 request_count는 성공한 동시 호출 수와 정확히 같아야 함")
                .isEqualTo(concurrentCalls);
          }
        }
        try (PreparedStatement ps =
            conn.prepareStatement("DELETE FROM rate_limit_counter WHERE bucket_key = ?")) {
          ps.setString(1, bucketKey);
          ps.executeUpdate();
        }
      }
  ...
  private int upsertResetOnNewWindowAndGetCount(Connection conn, String bucketKey)
