...
class AuthPolicyDdlShapeTest extends AbstractSchemaTest {
  ...
  void 신규_컬럼의_타입_NULL여부_기본값이_참조_DDL과_일치한다() throws Exception {
    withRollback(
        conn -> {
          ...
          assertColumn(
              conn, "terms_catalog", "effective_at", "timestamp with time zone", "NO", null);
          ...
          assertColumn(
              conn, "terms_agreement", "recorded_at", "timestamp with time zone", "NO", "now()");

          assertColumn(conn, "oauth_transaction", "member_id", "bigint", "YES", null);
          assertColumn(
              conn, "oauth_transaction", "initiating_session_id", "character varying", "YES", null);
          assertColumn(
              conn, "oauth_transaction", "expires_at", "timestamp with time zone", "NO", null);
          assertColumn(
              conn, "oauth_transaction", "used_at", "timestamp with time zone", "YES", null);

          assertColumn(
              conn, "kakao_signup_ticket", "provider_key", "character varying", "NO", null);
          assertColumn(
              conn, "kakao_signup_ticket", "expires_at", "timestamp with time zone", "NO", null);
  ...
  void 신규_FK의_컬럼_순서_참조_대상_삭제정책이_참조_DDL과_정확히_일치한다() throws Exception {
    withRollback(
        conn -> {
          ...
          assertConstraintDefinition(
              conn,
              "oauth_transaction_member_id_fkey",
    ...
    try (PreparedStatement ps =
      ...
      try (ResultSet rs = ps.executeQuery()) {
        ...
        assertThat(rs.getString("is_nullable"))
            .as("%s.%s NULL 허용 여부", table, column)
  ...
  private void assertConstraintDefinition(
      Connection conn, String constraintName, String expectedDefinition)
      throws java.sql.SQLException {
    try (PreparedStatement ps =
        conn.prepareStatement(
      ...
      try (ResultSet rs = ps.executeQuery()) {
        ...
        assertThat(actual)
            .as("제약 %s의 정의(컬럼 순서·참조 대상·삭제 정책 포함)", constraintName)
