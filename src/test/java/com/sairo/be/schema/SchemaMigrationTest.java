...
class SchemaMigrationTest extends AbstractSchemaTest {
    ...
    private static final List<String> DOMAIN_TABLES = List.of(
            "member", "login_identity", "login_history",
    ...
    private static final List<NamedObject> NAMED_CONSTRAINTS = List.of(
            ...
            new NamedObject("office_registration", "ck_office_registration_status", "CHECK (((((status)::text = ANY ((ARRAY['PENDING_REVIEW'::character varying, 'EVIDENCE_REQUESTED'::character varying])::text[])) AND (resulting_office_id IS NULL) AND (rejection_reason IS NULL) AND (reviewed_at IS NULL) AND (cancelled_at IS NULL) AND (reapplication_started_at IS NULL)) OR (((status)::text = 'APPROVED'::text) AND (resulting_office_id IS NOT NULL) AND (rejection_reason IS NULL) AND (reviewed_at IS NOT NULL) AND (cancelled_at IS NULL) AND (reapplication_started_at IS NULL)) OR (((status)::text = 'REJECTED'::text) AND (resulting_office_id IS NULL) AND (rejection_reason IS NOT NULL) AND (reviewed_at IS NOT NULL) AND (cancelled_at IS NULL) AND ((reapplication_started_at IS NULL) OR (reapplication_started_at > reviewed_at))) OR (((status)::text = 'CANCELLED'::text) AND (resulting_office_id IS NULL) AND (rejection_reason IS NULL) AND (reviewed_at IS NULL) AND (cancelled_at IS NOT NULL) AND (reapplication_started_at IS NULL))))"),
            new NamedObject("office_registration", "ck_office_registration_storage", "CHECK ((((status)::text <> ALL ((ARRAY['REJECTED'::character varying, 'CANCELLED'::character varying])::text[])) OR (document_object_key IS NULL) OR ((document_storage_status)::text = ANY ((ARRAY['MOVE_PENDING'::character varying, 'PENDING_DELETION'::character varying, 'DELETED'::character varying, 'MOVE_FAILED'::character varying])::text[]))))"),
            new NamedObject("office_registration_status_history", "ck_reg_history_actor", "CHECK (((((changed_by_type)::text = 'OPERATOR'::text) AND (changed_by_operator_id IS NOT NULL)) OR (((changed_by_type)::text = ANY ((ARRAY['APPLICANT'::character varying, 'SYSTEM'::character varying])::text[])) AND (changed_by_operator_id IS NULL))))"),
            new NamedObject("office_registration_status_history", "fk_reg_history_operator", "FOREIGN KEY (changed_by_operator_id) REFERENCES operator(id) ON DELETE RESTRICT"),
            new NamedObject("operator", "ck_operator_status", "CHECK (((((status)::text = 'ACTIVE'::text) AND (disabled_at IS NULL)) OR (((status)::text = 'DISABLED'::text) AND (disabled_at IS NOT NULL))))"),
            ...
            new NamedObject("property_contract", "ck_property_contract_status_by_deal_type", "CHECK ((((deal_type)::text <> 'SALE'::text) OR ((status)::text <> ALL ((ARRAY['RENEWED'::character varying, 'TERMINATED'::character varying])::text[]))))"),
    ...
    private static final List<NamedObject> NAMED_INDEXES = List.of(
            ...
            new NamedObject("login_identity", "uq_login_identity_member_provider_active", "CREATE UNIQUE INDEX uq_login_identity_member_provider_active ON public.login_identity USING btree (member_id, provider) WHERE (revoked_at IS NULL)"),
            ...
            new NamedObject("office_membership", "uq_office_membership_member_pending", "CREATE UNIQUE INDEX uq_office_membership_member_pending ON public.office_membership USING btree (member_id) WHERE ((status)::text = 'PENDING'::text)"),
            ...
            new NamedObject("office_registration", "uq_office_registration_pending_applicant", "CREATE UNIQUE INDEX uq_office_registration_pending_applicant ON public.office_registration USING btree (applicant_member_id) WHERE ((status)::text = ANY ((ARRAY['PENDING_REVIEW'::character varying, 'EVIDENCE_REQUESTED'::character varying])::text[]))"),
            new NamedObject("office_registration", "uq_office_registration_pending_bizno", "CREATE UNIQUE INDEX uq_office_registration_pending_bizno ON public.office_registration USING btree (business_registration_number) WHERE ((status)::text = ANY ((ARRAY['PENDING_REVIEW'::character varying, 'EVIDENCE_REQUESTED'::character varying])::text[]))"),
            ...
            new NamedObject("property", "ix_property_archived", "CREATE INDEX ix_property_archived ON public.property USING btree (office_id, archived_at) WHERE (deleted_at IS NULL)"),
            new NamedObject("operator", "uq_operator_kakao_provider_key", "CREATE UNIQUE INDEX uq_operator_kakao_provider_key ON public.operator USING btree (kakao_provider_key)"),
