...
class AuthPolicyConstraintTest extends AbstractSchemaTest {
  ...
  void 약관_버전이_draft_접두사면_카탈로그_등록이_거부된다() {
    assertConstraintViolation(
        "23514", conn -> insertTermsCatalog(conn, uniqueTermsCode(), "draft-2026-08-14"));
  }

  @Test
  void 약관_버전이_시행일_형식이_아니면_카탈로그_등록이_거부된다() {
    assertConstraintViolation("23514", conn -> insertTermsCatalog(conn, uniqueTermsCode(), "v1"));
  }

  @Test
  void 약관_버전의_월과_일_범위가_시행일_형식을_벗어나면_등록이_거부된다() {
    assertConstraintViolation(
        "23514", conn -> insertTermsCatalog(conn, uniqueTermsCode(), "2026-13-40"));
  }
  ...
  private void insertTermsAgreement(
      Connection conn, long memberId, String code, String version, boolean agreed)
      throws java.sql.SQLException {
    try (PreparedStatement ps =
        conn.prepareStatement(
