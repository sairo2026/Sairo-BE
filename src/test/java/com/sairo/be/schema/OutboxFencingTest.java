...
class OutboxFencingTest extends AbstractSchemaTest {
  ...
  void cleanup() throws Exception {
    try (Connection conn = dataSource.getConnection()) {
      if (officeRegistrationId != null) {
        exec(
            conn,
            "DELETE FROM document_move_outbox WHERE office_registration_id = ?",
  ...
  void lease가_만료되면_다른_워커가_dispatch_쿼리로_회수한다() throws Exception {
    try (Connection conn = dataSource.getConnection()) {
      ...
      exec(
          conn,
          "UPDATE document_move_outbox SET lock_expires_at = now() - interval '1 minute' WHERE id = ?",
          outboxId);
  ...
  private UUID acquireLease(Connection conn, long outboxId, String workerId)
  ...
  private int completeWithToken(Connection conn, long outboxId, UUID token)
      throws java.sql.SQLException {
    try (PreparedStatement ps =
        conn.prepareStatement(
  ...
  private int failWithToken(Connection conn, long outboxId, UUID token, String error)
      throws java.sql.SQLException {
    try (PreparedStatement ps =
        conn.prepareStatement(
  ...
  private long setupRejectedRegistrationWithPendingOutbox(Connection conn)
      throws java.sql.SQLException {
  ...
  private int attemptCount(Connection conn, long outboxId) throws java.sql.SQLException {
    try (PreparedStatement ps =
        conn.prepareStatement("SELECT attempt_count FROM document_move_outbox WHERE id = ?")) {
      ps.setLong(1, outboxId);
      try (ResultSet rs = ps.executeQuery()) {
        rs.next();
        return rs.getInt(1);
  ...
  private UUID lockToken(Connection conn, long outboxId) throws java.sql.SQLException {
    try (PreparedStatement ps =
        conn.prepareStatement("SELECT lock_token FROM document_move_outbox WHERE id = ?")) {
      ps.setLong(1, outboxId);
      try (ResultSet rs = ps.executeQuery()) {
        rs.next();
